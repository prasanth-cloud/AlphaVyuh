...
| Fix high/critical security findings. | No high/critical validated by the fallback scans. Four medium findings were fixed or gated; PR #72 adds broker provider error sanitization. | Done |
...
| Update launch docs with exact evidence. | `docs/public-launch-readiness-2026-05-07.md`, `docs/security-launch-scan-2026-05-07.md`, `docs/security-codex-scan-2026-05-07.md`, and this file. | Done |
...
| Push branch and open draft PR. | PR #71 was opened from the launch branch and merged after review: https://github.com/prasanth-cloud/AlphaVyuh/pull/71. Follow-up PR #72 carries the latest chart/security evidence: https://github.com/prasanth-cloud/AlphaVyuh/pull/72. | Done |
...
| Broker errors do not expose tokens/session material. | PR #72 sanitizes broker provider logs/responses and adds regression tests for sensitive provider messages. | Done |
...
| Supabase migrations reviewed and production-safe. | Repo migration docs reviewed; read-only staging advisors were run. New migration work remains owner-gated because the repo requires production-application evidence for migration PRs. | Owner-gated |
...
6. Add/apply/verify a Supabase advisor hardening migration for function search paths and direct EXECUTE grants in staging/prod.
