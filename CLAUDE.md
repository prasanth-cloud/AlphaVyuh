...
- **Known gaps:**
  ...
  - **[TODO] Write `scripts/deploy-migration.sh`.** Migration 033 required over an hour of manual CLI work due to a `supabase db push --db-url` auth/IP-ban loop, ultimately forcing an MCP fallback. A shell script wrapping the full staging→verify→prod→verify sequence into a single command would eliminate this toil. Spec: takes a migration number, runs `supabase migration list`, applies to staging, runs smoke-check queries, then applies to prod — aborting at each stage on failure. Required before the next migration that touches prod.
