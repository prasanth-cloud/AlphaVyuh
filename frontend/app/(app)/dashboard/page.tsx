  ...
  return (
    <div className="min-h-full bg-[#f2f2f0]">
      ...
      {market && (() => {
        ...
        return (
          <div
            className="mx-5 mt-4 mb-0 px-4 py-3 rounded-[10px] border flex items-center gap-3"
          ...
          >
            ...
            <span className="text-[12px] text-[#888]">{phase.desc}</span>
            <div className="ml-auto text-[11px] text-[#aaa] hidden sm:block">
              A/D {market.advance_decline_ratio?.toFixed(2)} · {market.new_52w_highs} new 52W highs · {market.above_ema200_pct}% above EMA 200
            </div>
          </div>
        );
      })()}
      ...
      <div className="grid grid-cols-4 gap-2.5 px-5 pt-3">
        {loading
          ? [1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-white border border-[#e8e8e6] rounded-[10px] px-4 py-3.5 animate-pulse">
          ...
          : [
              ...
              <div key={item.label} className="bg-white border border-[#e8e8e6] rounded-[10px] px-4 py-3.5">
                ...
                <div className="text-[10px] text-[#b0b0ae] uppercase tracking-wider mt-1">{item.label}</div>
              </div>
            ))}
      </div>
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="text-[11px] text-[#b0b0ae] mb-0.5">
          {new Date().toLocaleDateString("en-IN", {
            weekday: "long", year: "numeric", month: "long", day: "numeric",
          })}
        </div>
        <div className="text-[20px] font-semibold text-[#0f0f0e] tracking-tight">
          {greeting()}, {firstName}.
        </div>
        {error && (
          <div className="text-[12px] text-[#e5383b] mt-1">
          ...
          <div className="bg-white border border-[#e8e8e6] rounded-[10px] overflow-hidden">
            ...
            <div className="divide-y divide-[#f2f2f0]">
              {(moversTab === "gainers" ? movers.gainers
                ...
                >
                  <div>
                    <div className="text-[13px] font-semibold text-[#0f0f0e]">{row.symbol}</div>
        ...
        <div className="px-5 pb-4">
          <div className="bg-white border border-[#e8e8e6] rounded-[10px] overflow-hidden">
            <div className="px-4 py-3 border-b border-[#f0f0ee] flex items-center justify-between">
              <span className="text-[13px] font-bold text-[#0f0f0e]">Sector Breadth</span>
              ...
              {sectorBreadth.slice(0, 10).map(s => {
                ...
                return (
                  <div key={s.sector} className="flex items-center gap-3 px-4 py-2.5">
                    <div className="w-[130px] text-[12px] font-medium text-[#0f0f0e] truncate">{s.sector}</div>
                    {/* A/D bar */}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {movers.gainers.slice(0, 6).map(stock => (
              <Link
                ...
                className="flex-shrink-0 bg-white border border-[#e8e8e6] rounded-[10px] px-4 py-3 hover:border-[#bbb] transition-colors min-w-[110px]"
              >
                <div className="text-[13px] font-semibold text-[#0f0f0e]">{stock.symbol}</div>
      ...
      <div className="grid grid-cols-2 gap-2.5 px-5 pb-5">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className="bg-white border border-[#e8e8e6] rounded-[10px] p-[18px] hover:border-[#bbb] transition-colors block"
          >
            <div
            ...
            <div className="text-[14px] font-semibold text-[#0f0f0e] mb-0.5">{card.title}</div>
