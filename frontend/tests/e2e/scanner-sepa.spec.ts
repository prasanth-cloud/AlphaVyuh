...
test.describe("Scanner — SEPA preset", () => {
  ...
  test("SEPA preset chip is visible and first in the list", async ({ page }) => {
    ...
    // SEPA should be the first preset button
    const firstChip = page.locator("button", { hasText: /^SEPA$/ }).first();
    await expect(firstChip).toBeVisible();
  });

  test("clicking SEPA runs scan with correct filters and shows results", async ({ page }) => {
    ...
    // At least 1 result row
    const rows = page.locator("table tbody tr");
    await expect(rows).toHaveCount(1);

    // Verify the symbol rendered
    await expect(page.locator("table tbody")).toContainText("RELIANCE");

    // Verify the payload sent to the backend has the SEPA filters
  ...
  test("RS column renders value and highlights RS ≥ 70 in accent color", async ({
    page,
  }) => {
    await page.goto("/scanner");
    if (page.url().includes("/login")) return;
    ...
    // The RS value (82 in fixture) should be visible
    await expect(page.locator("table tbody")).toContainText("82");

    // The RS cell span should use --accent color for RS ≥ 70
    const rsSpan = page
      .locator("table tbody tr td")
      .filter({ hasText: /^82$/ })
      .locator("span");
    await expect(rsSpan).toHaveCSS("color", /rgb\(86, 215, 193\)|var\(--accent\)/);
  });
});
