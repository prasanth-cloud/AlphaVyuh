...
test.describe("Watchlist chart workspace failures", () => {
  test.skip(process.env.NEXT_PUBLIC_FORCE_LIVE_DATA !== "true", "Requires forced-live API mode to exercise failed workspace writes.");

  test("warns when indicator changes save locally but fail to sync", async ({ page }) => {
    let workspaceSaveRequests = 0;
    ...
    await page.route("**/api/v1/charts/**/workspace**", (route) => {
      if (route.request().method() === "POST") {
        workspaceSaveRequests += 1;
        return route.fulfill({
          status: 503,
          contentType: "application/json",
          body: JSON.stringify({ detail: "Chart workspace sync is temporarily unavailable." }),
        });
      }
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ symbol: "RELIANCE", timeframe: "D", indicators: [], drawings: [] }),
      });
    });
    ...
    await chartHeader.getByRole("button", { name: /EMA\(20\)|EMA 20/i }).click();

    const status = page.getByTestId("watchlist-chart-workspace-status");
    await expect(status).toContainText("saved locally but not synced", { timeout: 15_000 });
    await expect(status).toContainText("Chart workspace sync is temporarily unavailable.");
    expect(workspaceSaveRequests).toBe(1);
  });
});
