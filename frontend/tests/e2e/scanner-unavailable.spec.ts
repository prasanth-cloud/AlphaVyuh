...
test.describe("Scanner unavailable payloads", () => {
  ...
  test("does not treat scanner-created watchlists as populated when item adds fail", async ({ page }) => {
    let itemAddRequests = 0;
    await page.route("**/api/v1/scanner/screens", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await page.route("**/api/v1/watchlists**", (route) => {
      ...
      if (request.method() === "POST" && /\/api\/v1\/watchlists$/.test(new URL(request.url()).pathname)) {
        return route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({ id: "scanner-created", name: "Scanner queue", sort_order: 0, created_at: "2026-05-20T09:00:00Z", items: [] }),
        });
      }
      if (request.method() === "POST" && request.url().includes("/scanner-created/items")) {
        itemAddRequests += 1;
        return route.fulfill({
          status: 503,
          contentType: "application/json",
