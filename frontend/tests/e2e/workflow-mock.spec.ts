...
test.describe("Mock workflow smoke", () => {
  ...
  test("scanner shortlist creates a selected watchlist and chart drawing survives reload", async ({ page }) => {
    ...
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(1, { timeout: 10_000 });
    await page.reload();
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(1, { timeout: 15_000 });

    await page.getByTestId("chart-drawing-object").first().click({ force: true });
    await expect(page.getByText(/Selected: Trendline/i)).toBeVisible({ timeout: 10_000 });
    await page.keyboard.press("Delete");
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(0, { timeout: 10_000 });
    await page.reload();
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(0, { timeout: 15_000 });

    expect(errors).toEqual([]);
  });

  test("risk reward drawing stays selectable without chart trade-plan inputs", async ({ page }) => {
    test.setTimeout(60_000);
    const errors: string[] = [];
    page.on("console", (message) => {
      if (message.type() === "error") errors.push(message.text());
    ...
    await page.locator('select[aria-label="Tools"]').selectOption("LongPosition");
    ...
    await expect(page.getByText(/Use as plan/i)).toHaveCount(0);
  ...
  test("rectangle drawing can create a persisted zone note", async ({ page }) => {
    ...
    await page.locator('select[aria-label="Tools"]').selectOption("Rectangle");
    ...
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(2, { timeout: 10_000 });
    await expect(page.getByText(/Zone \d+\.\d+-\d+\.\d+/).first()).toBeVisible({ timeout: 10_000 });

    await page.reload();
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(2, { timeout: 15_000 });
  ...
  test("remaining full-chart drawing tools persist after reload", async ({ page }) => {
    ...
    const drawSegment = async (
      ...
      await page.mouse.up();
      await expect(page.getByTestId("chart-drawing-object")).toHaveCount(expectedCount, { timeout: 10_000 });
    };

    await drawSegment("Ray", /Ray armed/i, 0.22, 0.68, 0.58, 0.45, 1);
    await drawSegment("Horizontal", /Horizontal armed/i, 0.18, 0.50, 0.66, 0.50, 2);
    await drawSegment("HorizontalRay", /H-Ray armed/i, 0.26, 0.38, 0.62, 0.38, 3);
    await drawSegment("Fib", /Fib armed/i, 0.35, 0.72, 0.70, 0.32, 4);
    await drawSegment("ShortPosition", /Short Position armed/i, 0.42, 0.40, 0.64, 0.24, 5);

    await tools.selectOption("Text");
    ...
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(6, { timeout: 10_000 });
    await expect(page.getByText(/^Note$/).first()).toBeVisible({ timeout: 10_000 });

    await page.reload();
    await expect(page.getByTestId("chart-drawing-object")).toHaveCount(6, { timeout: 15_000 });
    await expect(page.getByText(/^Note$/).first()).toBeVisible({ timeout: 10_000 });
