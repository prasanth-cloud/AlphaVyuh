...
describe("market data error copy", () => {
  it("turns transport/backend outage errors into trader-facing copy", () => {
    expect(describeMarketDataError(new Error("404 Application not found"))).toContain("Live market data service is unavailable");
    expect(describeMarketDataError(new Error("Failed to fetch"))).toContain("Live market data service is unavailable");
  });
